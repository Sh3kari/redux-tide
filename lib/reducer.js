  ...
  return function() {
    ...
    if (action.type === _config.ACTION_EMPTY_TYPE_NAME) {
      return state.set(
        actionId,
        (0, _immutable.fromJS)(
          Object.assign({ entityKey: entityKey }, actionDefaultData, {
            time: time
          })
        )
      );
...
var makeNormalizedPayloadSource = function makeNormalizedPayloadSource(
  actionSchema,
  payloadSource,
  isArrayData,
  isFetching,
...
var makeEntitiesReducer = function makeEntitiesReducer(defaultEntitiesState) {
  return function() {
    ...
    var normalizedPayloadSource = makeNormalizedPayloadSource(
      actionSchema,
      payloadSource,
      isArrayData,
      isFetching,
    ...
    var newEntitiesItems = normalizedPayloadSource
    ...
    for (var entityName in newEntitiesItems) {
      for (var entityId in newEntitiesItems[entityName]) {
        ...
        var nextEntityState = (0, _immutable.fromJS)(
          newEntitiesItems[entityName][entityId]
        );
...
var createReducers = (exports.createReducers = function createReducers() {
  ...
  for (
    var _len = arguments.length, appSchema = Array(_len), _key = 0;
    _key < _len;
    _key++
  ) {
  ...
  var defaultEntitiesState = (0, _immutable.fromJS)(
    appSchema.reduce(function(memo, item) {
      memo[item.key] = {};
      return memo;
    }, {})
  );
  return (
    ...
    _defineProperty(
      _ref,
      _config.ACTIONS_REDUCER_NAME,
      makeActionsReducer(defaultActionsState)
    ),
    _defineProperty(
      _ref,
      _config.ENTITIES_REDUCER_NAME,
      makeEntitiesReducer(defaultEntitiesState)
    ),
    _ref
  );
});
